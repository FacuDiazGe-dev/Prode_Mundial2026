...
function parseFechaPartido(dia: unknown): string | null {
  ...
  const match = texto.match(/(\d{1,2})\s*[-/ ]\s*([a-zA-Z]{3})/);
...
function fechasPosiblesPartido(partido: EliminatoriaRow): string[] {
  const fechaApi = parseFechaPartido(partido.dia);
  if (!fechaApi) return [];

  const fechas = [fechaApi];
  const horaRaw = String(partido.hora || partido.hs || "");
  const hora = Number(horaRaw.split(":", 1)[0]);
...
async function leerPartidosEliminatoria(): Promise<EliminatoriaRow[]> {
  const supabaseUrl = requiredEnv("SUPABASE_URL");
  const serviceRoleKey = requiredEnv("SUPABASE_SERVICE_ROLE_KEY");
  const tableName = (Deno.env.get("SYNC_ELIMINATORIA_TABLE") || "resultados_eliminatoria").trim();
  ...
  return rows.filter((row: EliminatoriaRow) => {
    const equipo1 = normalizarTexto(row.equipo_1);
    const equipo2 = normalizarTexto(row.equipo_2);
    const placeholders = ["equipo 1", "equipo 2"];
    return Boolean(
      equipo1 &&
      equipo2 &&
      !placeholders.includes(equipo1) &&
      !placeholders.includes(equipo2) &&
      !equipo1.startsWith("ganador") &&
      !equipo2.startsWith("ganador") &&
      !equipo1.startsWith("perdedor") &&
      !equipo2.startsWith("perdedor")
    );
  });
}

function sugerirPartido(partido: EliminatoriaRow, matches: MatchApi[]) {
...
async function patchEliminatoria(nPartido: number, update: Record<string, unknown>) {
  const supabaseUrl = requiredEnv("SUPABASE_URL");
  const serviceRoleKey = requiredEnv("SUPABASE_SERVICE_ROLE_KEY");
  const tableName = (Deno.env.get("SYNC_ELIMINATORIA_TABLE") || "resultados_eliminatoria").trim();
  const url = new URL(`${supabaseUrl}/rest/v1/${tableName}`);
  url.searchParams.set("n_partido", `eq.${nPartido}`);
  ...
  if (!response.ok) {
    throw new Error(`Supabase update partido #${nPartido} respondio ${response.status}: ${await response.text()}`);
  }
}
...
async function actualizarEstadoVisual(sugerencia: ReturnType<typeof sugerirPartido>) {
  ...
  if (statusApi === "FINISHED") return null;
  if (!ESTADOS_AUTOMATICOS.has(sugerencia.estado)) return null;

  const esVisual = ["En Vivo", "Entretiempo"].includes(sugerencia.estado);
  const update = {
    estado_partido: sugerencia.estado,
    live: esVisual,
    live_r1: esVisual ? sugerencia.r1 : null,
    live_r2: esVisual ? sugerencia.r2 : null,
  };

  try {
    await patchEliminatoria(sugerencia.n_partido, update);
    return update;
  } catch (_error) {
    const updateAlt = {
      estado_partido: sugerencia.estado,
      live: esVisual,
      r1_live: esVisual ? sugerencia.r1 : null,
      r2_live: esVisual ? sugerencia.r2 : null,
    };
    await patchEliminatoria(sugerencia.n_partido, updateAlt);
    return updateAlt;
  }
}
...
Deno.serve(async (request) => {
  try {
    ...
    const partidos = await leerPartidosEliminatoria();
    ...
    for (const partido of partidos) {
      ...
      if (update) actualizados += 1;
      detalles.push({ ...sugerencia, update });
    }

    return Response.json({
      ok: true,
      tabla: (Deno.env.get("SYNC_ELIMINATORIA_TABLE") || "resultados_eliminatoria").trim(),
